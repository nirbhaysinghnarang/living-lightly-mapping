...
export const ChannelPopup: React.FC<ChannelPopupProps> = ({ channel, fixed }: ChannelPopupProps) => {
    function _renderContents() {
        return (<Stack direction="column" flex={1} justifyContent={"flex-start"} alignItems={"flex-start"}>
            <Typography variant="body1" sx={{ fontFamily: "Source Serif",color:"#191c1b", fontSize:"18px", width: "100%" }} gutterBottom>
                {channel.name}
            </Typography>
    ...
    if (!fixed && channel.contents.length == 0) return (
        <Popup anchor="top" closeOnClick={false}  closeButton={false} latitude={channel.lat} longitude={channel.long} offset={10} style={{ padding: 1, backgroundColor:"#f6f6f2" }}>
            {_renderContents()}
        </Popup>);

    if (!fixed) return (
        <Popup anchor="top" closeOnClick={false}  closeButton={false} latitude={channel.contents[0].lat} longitude={channel.contents[0].long} offset={10} style={{ padding: 1, backgroundColor:"#f6f6f2" }}>
            {_renderContents()}
        </Popup>);

    return <div style={{  position: "absolute", top: 100, left: 100, maxWidth: 300, opacity: 1, padding:"10px", backgroundColor:"#f6f6f2" }}>
        {_renderContents()}
    </div>
}
